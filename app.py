...
def ai_suggestions(
    score, attendance, study_hours, stress, health, participation, homework, sleep
):
    tips = []
    if attendance < 75:
        tips.append(
            "📅 **Attendance is critically low.** Target ≥85% to stay on track. Missing classes has a direct negative impact on final scores."
        )
    if study_hours < 2:
        tips.append(
            "📚 **Study hours are insufficient.** Aim for at least 3–4 focused hours/day. Use the Pomodoro technique for better retention."
        )
    if sleep < 6:
        tips.append(
            "😴 **Sleep deprivation detected.** Students sleeping <6 hrs score 15–20% lower on average. Aim for 7–8 hours nightly."
        )
    if stress >= 4:
        tips.append(
            "🧘 **High stress level detected.** Practice mindfulness, journaling, or yoga. Consider talking to a counselor."
        )
    if health <= 2:
        tips.append(
            "🏥 **Health status is poor.** Prioritize medical check-ups and a balanced diet — physical wellness strongly correlates with academic performance."
        )
    if participation <= 2:
        tips.append(
            "🙋 **Low classroom participation.** Active participation improves retention by up to 60%. Ask questions and join discussions."
        )
    if homework < 70:
        tips.append(
            "📝 **Homework completion is low.** Set a daily homework schedule. Consistent completion builds exam readiness."
        )
    if score >= 85:
        tips.append(
            "🌟 **Outstanding performance!** Keep the momentum — explore advanced topics, competitions, or mentorship roles."
        )
    elif score >= 70:
        tips.append(
            "✅ **Good progress!** Focus on weak subjects and aim for consistent improvement each week."
        )
    if not tips:
        tips.append(
            "🎯 All indicators look healthy! Keep maintaining this excellent routine."
        )
    return tips
...
def make_radar_chart(student_row, title="Student Radar"):
    ...
    fig, ax = plt.subplots(
        figsize=(5, 5), subplot_kw=dict(polar=True), facecolor="#1a1d27"
    )
...
if page == "🏠 Home Dashboard":
    ...
    cards = [
        ...
        (
            "🌟",
            top_student["Student_Name"].split()[0],
            "Top Performer",
            PALETTE["accent4"],
        ),
    ...
    with col1:
        ...
        fig_pie.update_layout(
            title=dict(
                text="Performance Category Breakdown",
    ...
    with col2:
        ...
        at_risk_df = (
            df[df["Final_Score"] < 60]
            .sort_values("Final_Score")[
                ["Student_Name", "Final_Score", "Attendance_%", "Performance_Category"]
            ]
            .head(8)
        )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        f"<p style='color:{PALETTE['muted']};margin-top:-12px;'>Search by name or UID for instant AI-powered insights.</p>",
        unsafe_allow_html=True,
    )

    search_query = st.text_input(
        "🔎 Type student name or UID...", placeholder="e.g. Navjot Kaur or 24108001"
    )
    ...
    if search_query:
        ...
        if results.empty:
            ...
            st.markdown(
                f"<p style='color:{PALETTE['accent2']};'>Found <b>{len(results)}</b> result(s):</p>",
                unsafe_allow_html=True,
            )
            ...
            for _, row in results.iterrows():
                ...
                rl, rc = risk_level(
                    pred, row["Attendance_%"], row["Study_Hours_Per_Day"]
                )
                cc = cat_color(cat)
                tips = ai_suggestions(
                    pred,
                    ...
                    row["Homework_Completion_%"],
                    row["Sleep_Hours"],
                )

                with st.expander(
                    f"👤 {row['Student_Name']}  —  UID: {row['UID']}  |  Score: {pred:.1f}",
                    ...
                    with c3:
                        ...
                        for tip in tips[:4]:
                            st.markdown(
                                f"<div style='font-size:.84rem;margin-bottom:8px;padding:8px 12px;background:{PALETTE['card2']};border-radius:8px;'>{tip}</div>",
                                unsafe_allow_html=True,
                            )

                    # Radar chart
                    fig_r = make_radar_chart(
                        row.to_dict(), title=f"{row['Student_Name']} — Profile Radar"
                    )
                    buf = io.BytesIO()
                    fig_r.savefig(
                        buf,
...
# ─────────────────────────────────────────────
    ...
    with tab2:
        ...
        with col2:
            ...
            attend_group = (
                df.groupby(attend_bins, observed=True)["Final_Score"]
                .mean()
                .reset_index()
            )
    ...
    with tab3:
        ...
        fig3 = px.bar(
            ...
            y="Final_Score",
            color="Final_Score",
            color_continuous_scale=["#00d4aa", "#ffd93d", "#ff6b6b"],
    ...
    with tab4:
        ...
        df_trend = df.copy()
        df_trend["Score_Delta"] = (
            df_trend["Final_Score"] - df_trend["Previous_Exam_Score"]
        )
        col1, col2 = st.columns(2)
        with col1:
            fig = px.scatter(
...
# ─────────────────────────────────────────────
    ...
    sort_col = st.selectbox(
        "Sort By",
        ["Final_Score", "Attendance_%", "Study_Hours_Per_Day", "Class_Test_Avg"],
    )

    top_df = (
        df.sort_values(sort_col, ascending=False).head(top_n).reset_index(drop=True)
    )
    top_df.index += 1
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        f"<p style='color:{PALETTE['muted']};margin-top:-12px;'>Compare two students side-by-side with radar charts and metrics.</p>",
        unsafe_allow_html=True,
    )
    ...
    if st.button("⚖️ Compare Students"):
        ...
        for col, row, name in [(rc1, r1, s1), (rc2, r2, s2)]:
            with col:
                ...
                buf = io.BytesIO()
                fig_r.savefig(
                    buf, format="png", dpi=120, bbox_inches="tight", facecolor="#1a1d27"
                )
                buf.seek(0)
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        f"<p style='color:{PALETTE['muted']};margin-top:-12px;'>Which factors matter most in predicting student performance?</p>",
        unsafe_allow_html=True,
    )
    ...
    colors_bar = [
        PALETTE["accent2"] if c > 0 else PALETTE["accent3"]
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        f"<p style='color:{PALETTE['muted']};margin-top:-12px;'>Upload a CSV with student data and get predictions for all students at once.</p>",
        unsafe_allow_html=True,
    )
    ...
    if uploaded:
        try:
            ...
            st.markdown(
                f"✅ Loaded **{len(upload_df)}** rows, **{len(upload_df.columns)}** columns."
            )
            ...
            if missing:
                ...
                upload_df["Performance_Category"] = upload_df["Predicted_Score"].apply(
                    score_to_cat
                )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        f"<p style='color:{PALETTE['muted']};margin-top:-12px;'>Enter details for a student not in the dataset and predict their performance.</p>",
        unsafe_allow_html=True,
    )
    ...
    with st.form("new_student_form"):
        ...
        with col2:
            ...
            extra = st.selectbox(
                "Extra Classes", [0, 1], format_func=lambda x: "Yes" if x else "No"
            )
            net = st.selectbox(
                "Internet Access", [0, 1], format_func=lambda x: "Yes" if x else "No"
            )
    ...
    if submitted:
        ...
        with col1:
            ...
            buf = io.BytesIO()
            fig_r.savefig(
                buf, format="png", dpi=130, bbox_inches="tight", facecolor="#1a1d27"
            )
            buf.seek(0)
            st.image(buf, use_container_width=True)
            plt.close(fig_r)
        with col2:
            ...
            for tip in tips:
                st.markdown(
                    f"<div style='padding:10px 14px;background:{PALETTE['card2']};border-radius:9px;margin-bottom:8px;font-size:.88rem;'>{tip}</div>",
                    unsafe_allow_html=True,
                )
